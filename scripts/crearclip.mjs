...
const p=await b.newPage(); await p.setViewport({width:1120,height:760})
...
await p.goto('http://localhost:5173/recetas/crear',{waitUntil:'networkidle0'})
await new Promise(r=>setTimeout(r,500))
// agregar 2 ingredientes
await p.evaluate(()=>{const btn=[...document.querySelectorAll('button')].find(b=>/Agregar ingrediente/i.test(b.textContent)); btn?.click(); btn?.click()})
await new Promise(r=>setTimeout(r,500))
await p.screenshot({path:OUT+'/crear-unidad.png'})
await b.close(); console.log('crear-unidad OK')
